#!/usr/bin/env bash
# ============================================================================
#  Jaddid Hayatak Android · management console · WINDOWS
...
SRC_BOOK_DIR="${SRC_BOOK_DIR:-$HOME/koutoub/02-jaddid-hayatak}"
KEYSTORE_DIR="${KEYSTORE_DIR:-$HOME/.keys}"
KEYSTORE_FILE="${KEYSTORE_FILE:-$KEYSTORE_DIR/wdiy-upload.keystore}"
APP_ID="${APP_ID:-org.workshopdiy.jaddidhayatak}"
AVD_NAME="${AVD_NAME:-jaddid_hayatak_test}"
...
cmd_test() {
  ...
  local out="/tmp/jaddid-hayatak-$(date +%Y%m%d-%H%M%S).png"
...
show_menu() {
  ...
  cat <<EOF
${BOLD}${MAGENTA}+==================================================================+
|   Jaddid Hayatak Android . management console (Windows $SHELL_ENV)${RESET}${BOLD}${MAGENTA}
